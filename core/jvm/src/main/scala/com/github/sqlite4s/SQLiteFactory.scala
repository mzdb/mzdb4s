...
object SQLiteFactory extends ISQLiteFactory with SQLiteConnectionFactory with SQLiteExceptionFactory {
  ...
  def configureLogging(logLevel: com.github.mzdb4s.LogLevel): Unit = {
    ...
    val javaLogLevel = logLevel match {
      ...
      case ERROR => JavaLogLevel.SEVERE
      case WARN => JavaLogLevel.WARNING
      case INFO => JavaLogLevel.INFO
      case DEBUG => JavaLogLevel.FINE
      case TRACE => JavaLogLevel.FINEST
    }

    JavaLogging.Logger.getLogger("com.almworks.sqlite4java").setLevel(javaLogLevel)
  }
}
...
class SQLiteBlobWrapper(val blob: SQLiteBlob) extends AnyVal with ISQLiteBlob {
  ...
  // FIXME: blob reopen is broken in sqlite4java (it throws an error "SQL logic error or missing database")
